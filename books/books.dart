...
void main() async {
  ...
  var book = '';
  try {
    var fBook = File(Directory.current.path + '/books/zdnet.txt');
    var bookLines = fBook.readAsLinesSync();

    var fOut = File(Directory.current.path + '/books/processed.txt');
    var fNew = File(Directory.current.path + '/books/new_words.txt');
    var fToLearn = File(Directory.current.path + '/books/to_learn.txt');
    ...
    var fBacklogWords = File(Directory.current.path + '/de/backlog_words.csv');
    ...
    for (var w in wordCount.keys) {
      wordIndex[w] = 100 * wordCount[w] + 10 * (backlogWords[w] ?? 0) - w.length;
    }
